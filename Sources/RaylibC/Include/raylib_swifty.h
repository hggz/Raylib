...
typedef struct Image {
    ...
    int format __attribute__((swift_private, swift_name("_format"))); // Data format (PixelFormat type)
} Image;
...
typedef struct NPatchInfo {
    ...
    int layout  __attribute__((swift_private, swift_name("_layout"))); // Layout of the n-patch: 3x3, 1x3 or 3x1
} NPatchInfo;
...
typedef struct Camera3D {
    ...
    int projection __attribute__((swift_private, swift_name("_projection"))); // Camera projection: CAMERA_PERSPECTIVE or CAMERA_ORTHOGRAPHIC
} Camera3D;
...
RLAPI void Draw3DBillboardRec(Camera camera, Texture2D texture, Rectangle source, Vector3 position, Vector2 size, Vector3 eulerAngles, Vector3 scale, Color tint); // Draw a billboard texture with full 3D
